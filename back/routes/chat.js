...
async function processFile(file) {
  ...
  try {
    ...
    if (result) {
      ...
      console.log(
        `✅ Fichier traité: ${file.name} en ${Date.now() - startTime}ms`
      );
    } else {
      return {
        fileName: file.name,
...
router.post("/", chatLimiter, validateChatInput, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🎯 Nouvelle requête [${requestId}] de ${userId}: ${message.substring(
        0,
        100
      )}${message.length > 100 ? "..." : ""}`
    );
    ...
    if (req.files) {
      ...
      if (req.files.file_0) {
        ...
        console.log(
          "File data length:",
          file.data ? file.data.length : "undefined"
        );
    ...
    if (!message && !uploadedFile) {
      return res.status(400).json({
        ...
        message:
          "Veuillez fournir un message ou des fichiers valides à analyser",
        requestId,
        timestamp: new Date().toISOString(),
      });
    }
    ...
    const intelligentResponse = await handleChatMessage(
      message,
      userId,
      uploadedFile
    );
    ...
    if (fileResults.length > 0) {
      ...
      response.fileStats = {
        ...
        avgProcessingTime: Math.round(
          fileResults.reduce((sum, f) => sum + (f.processingTime || 0), 0) /
            fileResults.length
        ),
      };
    }
    ...
    const errorResponse = {
      ...
      requestId,
      fallback:
        "Veuillez réessayer ou contacter le support Honest-Inn au 01 98 75 90 28",
      processingTime: `${Date.now() - startTime}ms`,
      timestamp: new Date().toISOString(),
      errorDetails:
...
router.get("/test", (req, res) => {
  res.json({
    ...
    features: {
      intelligentChat:
        "Système de chat intelligent avec mémoire de conversation",
